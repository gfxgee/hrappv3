...
return [
    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                /*
                 * Database-only backups: no application files are included.
                 */
                'include' => [],
        ...
        'destination' => [
            ...
            /*
             * The disk names on which the backups will be stored. Defaults to
             * S3-compatible object storage (Cloudflare R2) in production; set
             * BACKUP_DISK=local for local dev.
             */
            'disks' => [
                env('BACKUP_DISK', 's3'),
            ],
    ...
    'notifications' => [
        'notifications' => [
            // Only the problems are emailed; routine successes stay quiet (and
            // are visible on the Backups page). Mail fires only when a
            // BACKUP_NOTIFICATION_MAIL address is set.
    ...
    'cleanup' => [
        ...
        'default_strategy' => [
            /*
             * Keep every backup for 15 days; anything older is removed by
             * `backup:clean`. The tiered (daily/weekly/monthly/yearly) grading
             * is disabled so there is a simple, flat 15-day window. The most
             * recent backup is never deleted, even if it is older than 15 days.
             */
